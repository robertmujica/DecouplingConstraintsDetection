...
public class DecouplingMutatorFromClassConstraintXmlReader {
	
	private final String FILE_NAME = "DecouplingMutatorFromClassesConstraints.xml";
	
	/* 
	 * loads Constraints from a xml file 
	 * */
	public List<DecouplingMutatorFromClassConstraint> loadConstraints(){
		...
		try {
			  ...
			  for (int s = 0; s < nodeLst.getLength(); s++) {
			    ...
			    if (fstNode.getNodeType() == Node.ELEMENT_NODE) {
			    	...
			    	String forbiddenClassListAttribute = fstElmnt.getAttribute("forbiddenClassList");
			    	String[] forbiddenClassList = forbiddenClassListAttribute.split(";");
			    	...
			    	for(int i = 0; i < forbiddenClassList.length; i++){
			    		String allowedClassName = forbiddenClassList[i];
			    		forbiddenClasses.add(allowedClassName);
			    	}
			    	DecouplingMutatorFromClassConstraint constraint = new DecouplingMutatorFromClassConstraint(className, 
			    			mutators, forbiddenClasses, message);
			    	list.add(constraint);
			    }
